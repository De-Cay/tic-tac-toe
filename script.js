...
const CheckWinner = () => {
  ...
  initialize = (cellIndex, rowIndex, varient) => {
...
const TicTacToe = (() => {
  ...
  printVariantAndCheckValid = (target) => {
    ...
    const isPlayerWon = CheckWinner().initialize(cellIndex, rowIndex, Game.selectedVariant);
    if(isPlayerWon) {
      return setTimeout(() => {
        alert('You Won');
        startGame();
    ...
    const isComputerWon = CheckWinner().initialize(botIndeces.cell, botIndeces.row, Game.computerSelected);
    if(isComputerWon) {
      return setTimeout(() => {
        alert('You Loose');
        startGame();
...
document.addEventListener('DOMContentLoaded', TicTacToe.initialize, false);